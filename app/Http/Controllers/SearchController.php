    ...
    public function index(Request $request): Response
    {
        ...
        if (strlen(trim($q)) >= 2) {
            $userFavorites = auth()->check()
                ? \App\Models\YeuThich::where('nguoi_dung_id', auth()->id())
                    ->pluck('dich_vu_id')
                    ->toArray()
                : [];
            ...
            $results = DichVu::query()
                ...
                ->map(function ($s) use ($userFavorites) {
                    $image =
                        is_array($s->danh_sach_anh) &&
                        count($s->danh_sach_anh) > 0
                            ? $s->danh_sach_anh[0]
                    ...
                    return [
                        ...
                        'price' => (int) $s->gia_tu,
                        'rating' => round(
                            (float) ($s->nhaCungCap?->hoSoNhaCungCap
                                ?->diem_danh_gia ?? 0),
                            1,
                        ),
                        'reviews' => 0,
                        ...
                        'provider' =>
                            $s->nhaCungCap?->hoSoNhaCungCap?->ten_thuong_hieu ??
                            $s->nhaCungCap?->name,
