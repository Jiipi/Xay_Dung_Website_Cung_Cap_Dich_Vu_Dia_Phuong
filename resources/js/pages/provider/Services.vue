...
function toggleStatus(id: number) {
    ...
    router.post(
        `/provider/services/${id}/toggle-status`,
        {},
        {
            preserveScroll: true,
            onFinish: () => {
...
<template>
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <!-- Header Card -->
        <div
            class="overflow-hidden rounded-[2rem] border border-stone-200 bg-white shadow-sm"
        >
            <div
                class="flex flex-col gap-4 border-b border-stone-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between"
            >
                <div>
                    <p
                        class="text-sm font-semibold tracking-[0.2em] text-orange-700 uppercase"
                    >
                        Quản lý
                    </p>
                    <h1
                        class="mt-1 text-2xl font-black tracking-tight text-stone-950"
                    >
                        Dịch vụ của bạn
                    </h1>
                ...
                <div class="flex items-center gap-3">
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="w-52 rounded-xl border border-stone-200 bg-stone-50 py-2.5 pr-3 pl-9 text-sm transition-all outline-none focus:border-orange-400 focus:bg-white focus:ring-2 focus:ring-orange-100"
            ...
            <div
                class="grid gap-3 border-b border-stone-100 bg-stone-50/70 px-6 py-4 md:grid-cols-3"
            >
                <div
                    class="rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-600"
                >
                    <p class="font-semibold text-stone-900">Trạng thái duyệt</p>
                    <p class="mt-1">
                        Dịch vụ mới sẽ vào mục
                        <span class="font-medium text-amber-700"
                            >Chờ duyệt</span
                        >
                        trước khi hiển thị rộng rãi.
                    </p>
                </div>
                <div
                    class="rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-600"
                >
                    ...
                    <p class="mt-1">
                        Bạn có thể tạm ngưng để ẩn trên marketplace mà không cần
                        xóa hẳn.
                    </p>
                </div>
                <div
                    class="rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-600"
                >
                    <p class="font-semibold text-stone-900">Mục tiêu ưu tiên</p>
                    <p class="mt-1">
                        Tập trung vào dịch vụ đã duyệt nhưng booking thấp để cải
                        thiện chuyển đổi.
                    </p>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr
                            class="border-b border-stone-100 text-xs font-medium tracking-[0.16em] text-stone-400 uppercase"
                        >
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-stone-100"
                                    >
                                        <img
                                    ...
                                    <span
                                        class="max-w-[200px] truncate font-medium text-stone-950"
                                        >{{ svc.ten_dich_vu }}</span
                                    >
                                </div>
                            </td>
                            <td
                                class="px-6 py-4 whitespace-nowrap text-stone-600"
                            >
                                {{ svc.danh_muc }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-medium text-stone-950">{{
                                    formatVND(svc.gia_tu)
                                }}</span>
                                <span
                                    v-if="
                                        svc.gia_den &&
                                        svc.gia_den !== svc.gia_tu
                                    "
                                    class="text-stone-400"
                                >
                                    — {{ formatVND(svc.gia_den) }}</span
                                >
                            </td>
                            <td
                                class="px-6 py-4 whitespace-nowrap text-stone-600"
                            ...
                            <td class="px-6 py-4">
                                <span
                                    ...
                                    :class="
                                        duyetStyles[svc.trang_thai_duyet] ??
                                        'bg-stone-50 text-stone-600 ring-stone-200'
                                    "
                                >
                                    {{
                                        duyetLabels[svc.trang_thai_duyet] ??
                                        svc.trang_thai_duyet
                                    }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    ...
                                    :class="
                                        svc.trang_thai_hoat_dong === 'hoat_dong'
                                            ? 'text-emerald-600'
                                            : 'text-stone-400'
                                    "
                                >
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        :aria-label="
                                            svc.trang_thai_hoat_dong ===
                                            'hoat_dong'
                                                ? `Tạm ngưng ${svc.ten_dich_vu}`
                                                : `Kích hoạt ${svc.ten_dich_vu}`
                                        "
                                        :disabled="pendingStatusId === svc.id"
            ...
            >
                ...
                <p class="mt-1 text-xs text-stone-400">
                    Tạo dịch vụ đầu tiên để bắt đầu nhận booking
                </p>
                <Link
                    href="/provider/services/create"
            ...
            <div
                ...
                class="flex items-center justify-between border-t border-stone-100 px-6 py-4"
            >
                <p class="text-xs text-stone-500">
                    ...
                    {{
                        Math.min(
                            services.current_page * services.per_page,
                            services.total,
                        )
                    }}
                    / {{ services.total }} dịch vụ
    ...
    <Teleport to="body">
        ...
        >
            <div
                class="mx-4 w-full max-w-md rounded-[2rem] bg-white p-6 shadow-xl"
            >
